...
namespace Kaio.Core
{
    ...
    public static class ImageHelper
    {
        ...
        public static Bitmap Resize(byte[] source, int width, int height)
        {
            using (var _ms=new MemoryStream(source))
            {
                return Resize(_ms, width, height);
            }

         }

        public static Bitmap Resize(Stream source, int width, int height)
        {
        ...
        public static void ResizeStream(Stream source, string outputPath, int width, int height, string watermarkLogo, WatermarkPosition position, double opacity)
        {
            ...
            var _f = new FileInfo(outputPath);
            if (_f.Directory != null && !_f.Directory.Exists)
                _f.Directory.Create();
            _bitmap.Save(outputPath, _image.RawFormat);
            _graphic.Dispose();
            _bitmap.Dispose();
        ...
        public static bool Download(string url, string savePath)
        {
            ...
            request.UserAgent = "Mozilla/5.0 (Windows NT 6.2; WOW64; rv:10.0.2) Gecko/20100101 Firefox/10.0.2";
                ...
                _response.ContentType.StartsWith("image", StringComparison.OrdinalIgnoreCase))
            {
                ...
                try
                {
                    ...
                    var _f = new FileInfo(savePath);
                    if (_f.Directory != null && !_f.Directory.Exists)
                        _f.Directory.Create();

                    // if the remote file was found, download oit
                    using (Stream _inputStream = _response.GetResponseStream())
                    using (Stream _outputStream = File.OpenWrite(savePath))
                    {
                        var _buffer = new byte[4096];
                        int _bytesRead;
                        do
                        {
                            _bytesRead = _inputStream.Read(_buffer, 0, _buffer.Length);
                            _outputStream.Write(_buffer, 0, _bytesRead);
                        } while (_bytesRead != 0);
                    }
