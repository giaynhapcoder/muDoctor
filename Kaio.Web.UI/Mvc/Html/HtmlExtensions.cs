 ...
 */
            ...
            return enumerable.Select(f => new SelectListItem
                                              {
                                                  Value = value(f),
                                                  Text = text(f),
                                                  Selected = value(f).Equals(selectedValue)// String.Equals(value(f), selectedValue.ToString(), StringComparison.CurrentCultureIgnoreCase)
                                              });
        }
        ...
        public static ImageResult Image(this Controller controller, string imagePath, string etag)
        {

            return new ImageResult(imagePath, etag);
