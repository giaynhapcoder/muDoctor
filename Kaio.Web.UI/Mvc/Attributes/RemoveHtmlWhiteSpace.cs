      ...
      /*  public override void OnResultExecuting(ResultExecutingContext filterContext)
        {
            ...
            /*base.OnResultExecuting(filterContext);#1#

            var _response = filterContext.HttpContext.Response;
            if (!_response.IsRequestBeingRedirected)
                _response.Filter = new MinifiedStream(_response.Filter);
        }
*/
        ...
        public class MinifiedStream : MemoryStream
        {
            ...
            private static readonly Regex _JAVASCRIPT_TAGS = new Regex(@"<script[^>]*>([\w\W]*?)</script>", RegexOptions.Compiled | RegexOptions.IgnoreCase);
            ...
            private static string JavaScriptMatch(Match _m)
            {
                ...
                if (_m.Groups[1].Success && !string.IsNullOrEmpty(_m.Groups[1].Value))
                    return _m.ToString().Replace(_m.Groups[1].Value, string.Format("/*<![CDATA[*/{0}/*]]>*/", new ScriptPacker(ScriptPacker.PackerEncoding.Normal, true, false).Pack(_m.Groups[1].Value)));
                return _m.ToString();
            }
        }
