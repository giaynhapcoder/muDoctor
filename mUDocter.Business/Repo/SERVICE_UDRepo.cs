...
namespace mUDocter.Business.Repo
{
    ...
    public class SERVICE_UDRepo
    {
        public static void Save(SERVICE_UD obj)
        {
            if (obj.id > 0)
            {
                new MainDB().SERVICE_UD_Update(obj.id, obj.name, obj.dec, obj.orders, obj.status).Execute();
            }
            else
            {
                new MainDB().SERVICE_UD_Insert(obj.name, obj.dec, obj.orders, obj.status).Execute();
            }
        }
        ...
        public static List<SERVICE_UD> ListByDID()
        {
            return new MainDB().SERVICE_UD_SelectAll().ExecuteTypedList<SERVICE_UD>();
        }
