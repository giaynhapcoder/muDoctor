...
{    
    ...
    public class PROVINCE_UDRepo
    {
		public static void Save(PROVINCE_UD obj)
        {
		    						if (obj.id > 0)
            {
                new MainDB().PROVINCE_UD_Update(obj.id, obj.name, obj.orders, obj.created, obj.status).Execute();
            }
			else
			{
				new MainDB().PROVINCE_UD_Insert(obj.name, obj.orders, obj.created, obj.status).Execute();								
			}			
        }
